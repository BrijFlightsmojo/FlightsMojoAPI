    ...
    public class BookingController : ApiController
    {
        ...
        public HttpResponseMessage MakeBooking(string authcode, Core.Flight.OfflineBookingRequest request)
        {
            ...
            if (totPrice > 0 && !string.IsNullOrEmpty(SectorRef) && fsr.segment != null && fsr.segment.Count > 0)
            {
                ...
                else if (request.gds == Core.GdsType.Travelopedia)
                {
                    #region Travelopedia 
                    ServicesHub.Travelopedia.TravelopediaServiceMapping objTP = new ServicesHub.Travelopedia.TravelopediaServiceMapping();
                    Core.Flight.FlightSearchResponseShort result = objTP.GetFlightResults(fsr, true, false);
                    bookingLog(ref sbLogger, "Travelopedia Offline Booking result", JsonConvert.SerializeObject(result));
                    var selectedResult = result.Results[0].Where(k => k.ResultCombination.Equals(SectorRef, StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
                    ...
                    if (selectedResult != null && selectedResult.Fare != null)
                    {
                        ...
                        pvRequest.flightResult.Add(selectedResult);
                        Core.Flight.FareQuoteResponse pvResponse = objTP.GetFlightRePrice(pvRequest);
                        bookingLog(ref sbLogger, "Travelopedia Offline Booking FareQuoteResponse", JsonConvert.SerializeObject(pvResponse));
                        if (totPrice == pvResponse.VerifiedTotalPrice)
                        {
                            bookingRequest.AdminID = request.AdminID;
                            bookingRequest.adults = fsr.adults;
                            ...
                            bookingRequest.flightResult.Add(selectedResult);
                            Core.Flight.FlightBookingResponse bookingResponse = new Core.Flight.FlightBookingResponse(bookingRequest);
                            bookingLog(ref sbLogger, "Travelopedia Offline Booking bookingResponse", JsonConvert.SerializeObject(bookingResponse));

                            objTP.BookFlight(bookingRequest, ref bookingResponse);
                ...
                else if (request.gds == Core.GdsType.TripJack)
                {
                    #region TripJack 
                    ServicesHub.TripJack.TripJackServiceMapping objTripJack = new ServicesHub.TripJack.TripJackServiceMapping();
                    Core.Flight.FlightSearchResponseShort result = objTripJack.GetFlightResults(fsr);
                    bookingLog(ref sbLogger, "TripJack Offline Booking result", JsonConvert.SerializeObject(result));
                    var selectedResult = result.Results[0].Where(k => k.ResultCombination.Equals(SectorRef, StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
                    ...
                    if (selectedResult != null && selectedResult.Fare != null)
                    {
                        ...
                        pvRequest.flightResult.Add(selectedResult);
                        Core.Flight.FareQuoteResponse pvResponse = objTripJack.GetFlightReview(pvRequest);
                        bookingLog(ref sbLogger, "TripJack Offline Booking FareQuoteResponse", JsonConvert.SerializeObject(pvResponse));
                        if (totPrice == pvResponse.VerifiedTotalPrice)
                        {
                            bookingRequest.AdminID = request.AdminID;
                            bookingRequest.adults = fsr.adults;
                            ...
                            bookingRequest.flightResult.Add(selectedResult);
                            Core.Flight.FlightBookingResponse bookingResponse = new Core.Flight.FlightBookingResponse(bookingRequest);
                            bookingLog(ref sbLogger, "TripJack Offline Booking bookingResponse", JsonConvert.SerializeObject(bookingResponse));

                            objTripJack.BookFlightCRM(bookingRequest, ref bookingResponse);

                            DAL.Booking.SaveBookingDetails objSaveBookingDetails = new DAL.Booking.SaveBookingDetails();
                            objSaveBookingDetails.SaveFMJ_FlightBookingTransactionDetailsWithTickNo(ref bookingRequest, ref bookingResponse);
                            new ServicesHub.LogWriter_New(sbLogger.ToString(), bookingRequest.bookingID.ToString(), "OfflineBooking", "TripJack Offline BookFlight Original Response");
                        }
                        else
                        {
                            res.status = Core.TransactionStatus.Error;
                            ...
                            bookingLog(ref sbLogger, "TripJack Offline Booking Else Increase price", res.message);
                            new ServicesHub.LogWriter_New(sbLogger.ToString(), request.BookingID.ToString(), "OfflineBooking", "TripJack Offline BookFlight Original Response");
                    ...
                    else
                    {
                        ...
                        bookingLog(ref sbLogger, "TripJack Offline Booking Else Same fare not available", res.message);
                        new ServicesHub.LogWriter_New(sbLogger.ToString(), request.BookingID.ToString(), "OfflineBooking", "TripJack Offline BookFlight Original Response");
                    }
